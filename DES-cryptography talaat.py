...
# Initial Permutation for Blocks
def initial_permutation(block):
    return [block[i - 1] for i in IP]


# Final Permutation for Blocks
def final_permutation(block):
    return [block[i - 1] for i in FP]


# Expansion Permutation for Right Half
def expansion_permutation(right_half):
    return [right_half[i - 1] for i in E]
...
def s_box_substitution(block):
    ...
    for i in range(0, len(block), 6):
        ...
        s_boxed.extend([int(bit) for bit in format(value, '04b')])
    return s_boxed
...
def des_round(left, right, key):
    ...
    xor_result = xor_operation(expanded_right, key)
    s_boxed = s_box_substitution(xor_result)
    permuted = [s_boxed[i - 1] for i in P]
...
def generate_subkeys(key):
    ...
    key = [int(bit) for bit in ''.join(format(ord(char), '08b') for char in key)]

    # Apply Permutation Choice 1
    key = permutation_choice_1(key)
    subkeys = [key]

    for i in range(1, 17):
        key = left_shift(key[:28], KEY_SHIFTS[i - 1]) + left_shift(key[28:], KEY_SHIFTS[i - 1])
        subkeys.append(permutation_choice_2(key))
...
def des_encrypt(plain_text, key):
    plain_text = [int(bit) for bit in ''.join(format(ord(char), '08b') for char in plain_text)]
...
def des_decrypt(cipher_text, key):
    cipher_text = [int(bit) for bit in cipher_text]
    ...
    decrypted_text = final_permutation(right + left)
    return binary_to_string(map(str, decrypted_text))
# Key Shifts for Each Round
KEY_SHIFTS = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]

# P-Box Permutation for S-Box Output
...
def main():
    ...
    if len(plain_text) < 8:
        # Handle adding special characters
        plain_text += '@' * (8 - len(plain_text))
    elif len(plain_text) > 8:
        # Handle dividing plain text into blocks of size 8
    ...
    print(f"Output2 from decryption: {decrypted_text}")
